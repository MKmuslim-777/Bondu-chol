import { FaFacebook, FaGithub, FaEnvelope, FaExternalLinkAlt } from "react-icons/fa";
  ...
  return (
    <div className="group relative bg-white dark:bg-slate-900 rounded-[2.5rem] overflow-hidden shadow-xl hover:shadow-[0_20px_50px_rgba(0,0,0,0.1)] dark:hover:shadow-[0_20px_50px_rgba(59,130,246,0.1)] transition-all duration-700 border border-slate-100 dark:border-slate-800 hover:-translate-y-3">
      ...
      <div className="absolute top-12 left-1/2 -translate-x-1/2">
        <div className="avatar ring-[6px] ring-white dark:ring-slate-900 rounded-full shadow-2xl group-hover:ring-primary/20 transition-all duration-500">
          <div className="w-28 rounded-full bg-slate-200 relative overflow-hidden">
            ...
            <img src={friend.photoURL || "https://ui-avatars.com/api/?name=User"} className="group-hover:scale-110 transition-transform duration-700" alt={friend.displayName} />
          </div>
        </div>
      ...
      <div className="pt-16 pb-10 px-8 text-center">
        <div className="mb-6">
          <h3 className="text-2xl font-black text-slate-800 dark:text-white uppercase tracking-tighter group-hover:text-primary transition-colors">{friend.displayName}</h3>
          <div className="inline-block mt-1 px-4 py-1 bg-primary/10 text-primary text-[10px] font-black rounded-full border border-primary/20">{friend.role || "MEMBER"}</div>
        </div>
        ...
        <div className="space-y-3 mb-8">
          <div className="flex items-center gap-3 p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 hover:border-primary/30 transition-colors">
            <div className="p-2 bg-white dark:bg-slate-800 shadow-sm rounded-lg text-primary"><FaEnvelope className="text-sm" /></div>
            <p className="text-xs font-bold text-slate-600 dark:text-slate-300 truncate tracking-tight italic">{friend.email}</p>
          </div>
        </div>

        <div className="flex items-center justify-between pt-6 border-t border-slate-100 dark:border-slate-800">
          <div className="flex gap-2">
            <a href="#" className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800 text-blue-600 hover:bg-blue-600 hover:text-white hover:rotate-12 transition-all shadow-sm border border-slate-100 dark:border-slate-800"><FaFacebook /></a>
            <a href="#" className="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800 text-slate-900 dark:text-white hover:bg-black hover:-rotate-12 transition-all shadow-sm border border-slate-100 dark:border-slate-800"><FaGithub /></a>
          </div>
